...
minikube dashboard
kubectl create namespace oracle-jdbc-shared-store
kubectl config set-context --current --namespace=oracle-jdbc-shared-store
```
...
kubectl apply -f oracle-jdbc-shared-store/ext-acceptor-minikube-install.yaml
```

## Hosts
```
export EXT_ACCEPTOR_HOST=$(kubectl get ingress ext-acceptor-svc-ing -o json | jq -r '.spec.rules[].host')
export PRIMARY_BROKER_EXT_ACCEPTOR_HOST=$(kubectl get ingress primary-broker-ext-acceptor-0-svc-ing -o json | jq -r '.spec.rules[].host')
export PRIMARY_BROKER_CONSOLE_HOST=$(kubectl get ingress primary-broker-wconsj-0-svc-ing -o json | jq -r '.spec.rules[].host')
export BACKUP_BROKER_EXT_ACCEPTOR_HOST=$(kubectl get ingress backup-broker-ext-acceptor-0-svc-ing -o json | jq -r '.spec.rules[].host')
export BACKUP_BROKER_CONSOLE_HOST=$(kubectl get ingress backup-broker-wconsj-0-svc-ing -o json | jq -r '.spec.rules[].host')

echo "$(minikube ip) ${EXT_ACCEPTOR_HOST}" | sudo tee -a /etc/hosts
echo "$(minikube ip) ${PRIMARY_BROKER_EXT_ACCEPTOR_HOST}" | sudo tee -a /etc/hosts
echo "$(minikube ip) ${PRIMARY_BROKER_CONSOLE_HOST}" | sudo tee -a /etc/hosts
echo "$(minikube ip) ${BACKUP_BROKER_EXT_ACCEPTOR_HOST}" | sudo tee -a /etc/hosts
echo "$(minikube ip) ${BACKUP_BROKER_CONSOLE_HOST}" | sudo tee -a /etc/hosts
```
